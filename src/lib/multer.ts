...
// Filter photos
const filterFotos = async (req: Request, file: any, cb: any) => {
  const filetypes = /JPG|JPEG|jpg|jpeg|png|PNG/;
  const mimetype = filetypes.test(file.mimetype);
  const extname = filetypes.test(path.extname(file.originalname));
  if (mimetype && extname) return cb(null, true);
  return cb('Archivo debe ser una foto.');
};
...
// Filter documents
const filterDocuments = async (req: Request, file: any, cb: any) => {
  const filetypes = /DOC|doc|DOCX|docx|xls|XLS|XLSX|xlsx|ppt|PPT|PPTX|pptx|PDF|pdf/;
  const mimetype = filetypes.test(file.mimetype);
  const extname = filetypes.test(path.extname(file.originalname));
  if (mimetype && extname) return cb(null, true);
  return cb('Archivo debe ser un documento docx,xlsx,pptx,pdf.');
};
